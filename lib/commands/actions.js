...
const createProjectAction = async (project, others) => {
  ...
  try {
    await new Promise((resolve, reject) => {
      download(templateAddress, project, {
        clone: false
      }, function (err) {
        if (err) {
  ...
  if (warehouse === 'react') {
    const yarnOptions = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'useYarn',
        message: '您当前下载的是react模板,是否使用yarn来操作？（如果您电脑没有安装yarn,cli会帮您自动安装）',
        default: true
      },
    ])
    ...
    if (yarnOptions.useYarn) {
      ...
      try {
        ...
        await commandSpawn(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['install', 'yarn', '-g',])
      }
    }
  }
